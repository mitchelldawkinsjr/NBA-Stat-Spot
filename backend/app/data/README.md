...
**Finding missing players:** Call `GET /api/v1/admin/players/missing-from-espn` to list names on ESPN rosters that aren’t in the app. Look up each player on NBA.com to get their `nba_id`, then add an entry here. You can also run `cd backend && PYTHONPATH=. .venv/bin/python scripts/check_missing_rookies.py` to compare NBA CommonAllPlayers and ESPN vs the app and get suggested entries (when the NBA API responds it may include `nba_id`).
