...
export default function PlayerTrends() {
  ...
  function Chart({ keys, title }: { keys: string[], title: string }) {
    return (
      <div className="border border-gray-200 dark:border-outline-variant/30 rounded-lg p-3 bg-surface-container transition-colors duration-200">
        <div className="mb-2 font-semibold text-gray-900 dark:text-on-surface transition-colors duration-200">{title}</div>
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8">
      <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-on-surface mb-4 transition-colors duration-200">Player Trends</h2>
      <div className="flex flex-wrap gap-3 items-center mb-3">
        ...
        <input 
          ...
          className="px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-surface-container-high text-gray-900 dark:text-on-surface focus:outline-none focus:ring-2 focus:ring-blue-600/20 dark:focus:ring-blue-400/20 transition-colors duration-200"
        />
      </div>
      <div className="text-xs text-on-surface-variant mb-4 transition-colors duration-200">Rolling averages: 5-game and 10-game overlays for quick trend reading.</div>
      {player && (
