...
export function TrendChart({ title, data, color, yLabel, ariaLabel, showRollingAverages = true }: TrendChartProps) {
  ...
  const CustomTooltip = ({ active, payload }: TooltipData) => {
    if (active && payload && payload.length && payload[0]?.payload) {
      ...
      return (
        <div className="bg-surface-container border border-outline/20 rounded-lg shadow-lg p-3 transition-colors duration-200">
          <p className="text-xs font-semibold text-on-surface mb-1 transition-colors duration-200">
            Game {data.idx}
          </p>
          {data.date && (
            <p className="text-xs text-on-surface-variant mb-2 transition-colors duration-200">{data.date}</p>
          )}
          {data.matchup && (
            <p className="text-xs text-on-surface-variant mb-2 transition-colors duration-200">{data.matchup}</p>
          )}
          <div className="space-y-1">
            <div className="flex items-center justify-between gap-4">
              <span className="text-xs text-on-surface-variant transition-colors duration-200">Actual:</span>
            ...
            {data.rolling5 !== null && (
              <div className="flex items-center justify-between gap-4">
                <span className="text-xs text-on-surface-variant transition-colors duration-200">5-Game Avg:</span>
            ...
            {data.rolling10 !== null && (
              <div className="flex items-center justify-between gap-4">
                <span className="text-xs text-on-surface-variant transition-colors duration-200">10-Game Avg:</span>
            ...
            <div className="flex items-center justify-between gap-4">
              <span className="text-xs text-on-surface-variant transition-colors duration-200">Prop Line:</span>
              <span className="text-xs font-bold" style={{ color: danger }}>
                {data.propLine}
              </span>
            </div>
            <div className="flex items-center justify-between gap-4 pt-1 border-t border-outline/20 transition-colors duration-200">
              <span className="text-xs text-on-surface-variant transition-colors duration-200">Result:</span>
              <span className={`text-xs font-bold ${data.isOver ? 'text-betting-green' : 'text-error'} transition-colors duration-200`}>
  ...
  return (
    <div className="rounded bg-surface-container border border-outline/20 p-4 shadow-sm transition-colors duration-200">
      <div className="flex items-center justify-between mb-3">
        <div className="text-sm font-semibold text-on-surface transition-colors duration-200">{title}</div>
        <div className="flex items-center gap-3 text-xs text-on-surface-variant transition-colors duration-200">
      ...
      {/* Stats summary */}
      <div className="mt-3 pt-3 border-t border-outline/20 grid grid-cols-3 gap-2 text-xs transition-colors duration-200">
        <div>
          <div className="text-on-surface-variant transition-colors duration-200">Avg</div>
          <div className="font-semibold text-on-surface transition-colors duration-200">{overallAvg.toFixed(1)}</div>
        </div>
        <div>
          <div className="text-on-surface-variant transition-colors duration-200">Over Line</div>
          <div className="font-semibold text-betting-green transition-colors duration-200">
            {enrichedData.filter(d => d.isOver).length}/{enrichedData.length}
          </div>
        </div>
        <div>
          <div className="text-on-surface-variant transition-colors duration-200">Hit Rate</div>
          <div className="font-semibold text-primary-container transition-colors duration-200">
